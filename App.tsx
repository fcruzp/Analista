        ...
        <div className="max-w-3xl mx-auto print:max-w-none">
          ...
          <div className="prose prose-invert prose-cyan max-w-none print:text-black print:prose-neutral">
            {analysis.content.split('\n').map((line, i) => {
              if (line.startsWith('# ')) return <h1 key={i} className="text-2xl md:text-4xl font-black mb-8 border-b border-slate-800 pb-4 text-white print:text-black print:border-black leading-tight uppercase italic">{line.replace('# ', '')}</h1>;
              ...
              return (
                <p key={i} className="mb-5 text-slate-300 print:text-black leading-relaxed text-sm md:text-lg font-light whitespace-pre-wrap tracking-wide">
                  {parts.map((part, j) => {
      ...
      {showTeleprompter && (
        ...
        >
          ...
          <div className="max-w-4xl w-full h-[75vh] overflow-hidden relative border-y border-white/5 cursor-pointer" onClick={() => setIsPaused(!isPaused)}>
            ...
            >
              {analysis.content.split('\n').map((line, i) => {
                if (line.startsWith('# ')) return <div key={i} className="text-4xl md:text-7xl font-black text-white mb-10 uppercase italic">{line.replace('# ', '')}</div>;
                if (line.startsWith('## ')) return <div key={i} className="text-3xl md:text-6xl font-black text-cyan-400 mt-14 mb-7 uppercase tracking-[0.1em]">{line.replace('## ', '')}</div>;
                if (line.startsWith('### ')) return <div key={i} className="text-2xl md:text-5xl font-black text-slate-400 mt-10 mb-5 uppercase tracking-widest">{line.replace('### ', '')}</div>;
            ...
            return (
              <div key={item.id} className="group bg-slate-900/40 border border-slate-800/80 rounded-2xl p-6 flex flex-col hover:border-cyan-500/50 transition-all relative overflow-hidden backdrop-blur-sm">
                ...
                <div className="mt-auto flex justify-between items-center relative z-10">
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => onViewItem(item)} className="p-2.5 bg-slate-800 hover:bg-cyan-600/20 rounded-xl text-slate-400 hover:text-cyan-400 transition-all border border-slate-700">
      ...
      <div className="space-y-6 flex-1">
        ...
        <div className="flex justify-center md:justify-end gap-4 pt-8 pb-20 md:pb-12">
          <button
            ...
            className={`w-full md:w-auto px-12 py-4 rounded-xl font-black text-xs uppercase tracking-[0.3em] transition-all shadow-xl active:scale-95 ${saveStatus === 'saved' ? 'bg-green-600 text-white shadow-green-500/20' : 'bg-cyan-600 text-white hover:bg-cyan-500 shadow-cyan-500/20'
              }`}
          >
