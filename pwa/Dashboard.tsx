      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-left">
        ...
        <div className="bg-slate-900/40 border border-slate-800 p-4 md:p-6 rounded-2xl md:col-span-2 shadow-2xl backdrop-blur-md">
          ...
          <div className="space-y-3">
            {loading ? (
              [1, 2, 3].map(i => <div key={i} className="h-20 bg-slate-900/50 rounded-xl animate-pulse border border-slate-800/50"></div>)
            ) : needsKey ? (
              ...
              <>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-3 ml-4 relative z-10">
                        ...
                        <span className={`text-[8px] font-black px-2.5 py-1.5 rounded-lg tracking-[0.2em] hidden sm:block border ${
                          item.impact === 'Alto' ? 'bg-red-500/10 text-red-500 border-red-500/30' : 
                ...
                {/* NODOS DE VERIFICACIÓN - Agregados inmediatamente después de la lista */}
                {sources.length > 0 && (
                  <div className="mt-8 pt-6 border-t border-slate-800/40 animate-fade-in-up">
                    <div className="flex items-center gap-2 mb-4 ml-1">
                      <div className="w-1 h-1 rounded-full bg-cyan-500 shadow-[0_0_8px_rgba(34,211,238,1)]"></div>
                      <p className="text-[8px] font-black text-cyan-500/60 uppercase tracking-[0.4em] font-mono">Nodos de Verificación</p>
                    </div>
                    <div className="flex flex-wrap gap-2">
                      {sources.map((source, i) => (
                        <a 
                          ...
                          target="_blank" 
                          rel="noopener noreferrer" 
                          className="group flex items-center gap-2 text-[8px] font-black bg-slate-950/80 text-slate-500 hover:text-cyan-400 px-3 py-2 rounded-lg border border-slate-800 hover:border-cyan-500/40 transition-all shadow-sm backdrop-blur-sm"
                        >
                          <span className="opacity-40 group-hover:opacity-100 transition-all text-slate-400 group-hover:text-cyan-400">
                            <Icons.ExternalLink />
                          </span>
                          <span className="truncate uppercase tracking-widest max-w-[120px]">{source.title}</span>
                        </a>
                      ))}
                    </div>
                  </div>
